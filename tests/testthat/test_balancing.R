...
gamma.pogs.free2 = approx.balance(M, balance.target = balance.target, optimizer = "pogs.dual", allow.negative.weights = TRUE, zeta = 0.5)
...
test_that("optimizers match", {
  ...
  expect_equal(max(abs(gamma.qp.free - gamma.pogs.free)), 0, tolerance = 5e-2)
