...
test_that("positivity constraint works", {
  expect_true(all(gamma.pogs.positive > -10^(-5)))
...
test_that("gamma sums to 1", {
  expect_equal(sum(gamma.positive), 1, tolerance = 5e-3)
  expect_equal(sum(gamma.free), 1, tolerance = 5e-5)
...
test_that("optimizers match", {
  ...
  expect_equal(max(abs(gamma.qp.positive - gamma.pogs.positive)), 0, tolerance = 5e-3)
})
