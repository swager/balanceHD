...
gamma.mosek.positive = approx.balance(M, balance.target = balance.target, optimizer = "mosek", allow.negative.weights = FALSE, use.dual = FALSE, zeta = 0.5)
imbalance.mosek.positive = t(M) %*% gamma.mosek.positive - balance.target

gamma.mosek.free = approx.balance(M, balance.target = balance.target, optimizer = "mosek", allow.negative.weights = TRUE, use.dual = FALSE, zeta = 0.5)
imbalance.mosek.free = t(M) %*% gamma.mosek.free - balance.target

gamma.mosek.positive2 = approx.balance(M, balance.target = balance.target, optimizer = "mosek", allow.negative.weights = FALSE, use.dual = TRUE, zeta = 0.5)
imbalance.mosek.positive2 = t(M) %*% gamma.mosek.positive - balance.target

gamma.mosek.free2 = approx.balance(M, balance.target = balance.target, optimizer = "mosek", allow.negative.weights = TRUE, use.dual = TRUE, zeta = 0.5)
imbalance.mosek.free2 = t(M) %*% gamma.mosek.free2 - balance.target

gamma.pogs.positive = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = FALSE, use.dual = FALSE, zeta = 0.5)
imbalance.pogs.positive = t(M) %*% gamma.pogs.positive - balance.target

gamma.pogs.free = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = TRUE, use.dual = FALSE, zeta = 0.5)
imbalance.pogs.free = t(M) %*% gamma.pogs.free - balance.target

gamma.pogs.positive2 = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = FALSE, use.dual = TRUE, zeta = 0.5)
imbalance.pogs.positive2 = t(M) %*% gamma.pogs.positive2 - balance.target

gamma.pogs.free2 = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = TRUE, use.dual = TRUE, zeta = 0.5)
...
test_that("removing positivity constraint improves objective", {
  ...
  expect_true((sum(gamma.mosek.positive2^2) + max(abs(imbalance.mosek.positive2))^2) >
                (sum(gamma.mosek.free2^2) + max(abs(imbalance.mosek.free2))^2))
...
test_that("gamma sums to 1", {
  ...
  expect_equal(sum(gamma.mosek.positive2), 1)
  expect_equal(sum(gamma.mosek.free2), 1)
  expect_equal(sum(gamma.pogs.positive), 1, tolerance = 5e-3)
  expect_equal(sum(gamma.pogs.free), 1, tolerance = 5e-4)
...
gamma.mosek.free2.zeta9 = approx.balance(M, balance.target = balance.target, optimizer = "mosek", allow.negative.weights = TRUE, use.dual = TRUE, zeta = 0.9)
imbalance.mosek.free2.zeta9 = t(M) %*% gamma.mosek.free2.zeta9 - balance.target

gamma.pogs.free.zeta9 = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = TRUE, use.dual = TRUE, zeta = 0.9)
imbalance.pogs.free.zeta9 = t(M) %*% gamma.pogs.free.zeta9 - balance.target

gamma.pogs.free2.zeta9 = approx.balance(M, balance.target = balance.target, optimizer = "pogs", allow.negative.weights = TRUE, use.dual = FALSE, zeta = 0.9)
...
test_that("zeta tunes problem in right direction", {
  ...
  expect_true(max(abs(imbalance.mosek.free2.zeta9)) < max(abs(imbalance.mosek.free2)))
  expect_true(sum(gamma.mosek.free2.zeta9^2) > sum(gamma.mosek.free2^2))
